...
function createSlideList(start,end){
  var list = [];
  for (i = start; i < (end+1); i++){
     list.push( 'img/ins/Slide ' + i + 'H.png');}
  return list;
}
...
function getFaceSample (){  //get the sample of faces in each trial
  ...
  Face.emotionX = getRandomElement([0,50, 100]); //randomly choose from negative and postive emotion
...
//ee
  ...
  ['img/'+ Face.personX +(Face.emotionX + Face.pos[1]) + '.jpg', 'img/'+ Face.personX +(Face.emotionX + Face.pos[2]) + '.jpg',
   'img/'+ Face.personX +(Face.emotionX + Face.pos[3]) + '.jpg'],
  ['img/'+ Face.personX +(Face.emotionX + Face.pos[4]) + '.jpg','img/'+ Face.personX +(Face.emotionX + Face.pos[5]) + '.jpg',
  'img/'+ Face.personX +(Face.emotionX + Face.pos[6]) + '.jpg'],
   ['img/'+ Face.personX +(Face.emotionX + Face.pos[7]) + '.jpg', 'img/'+ Face.personX +(Face.emotionX + Face.pos[8]) + '.jpg',
  'img/'+ Face.personX +(Face.emotionX + Face.pos[9]) + '.jpg']
  //, 'img/'+ Face.personX +(Face.emotionX + Face.pos[10])+ '.jpg',
...
function morphedScale (){ //generate the rating scale depending on the person and valence randomly chosen in faceArray
  ...
  var linePrompt = `<div id="linePrompt"><div style="font-size:50px;position:absolute;margin-left:${lineSlice*1.3}px;margin-top:${vHeight/2}px">⬅</div><div style="position:absolute;margin-left:${lineSlice*1.72}px;margin-top:${vHeight/2}px;z-index:5;">הזז את העכבר מעבר לקו כדי להתחיל</div></div>`
