/**
Various useful functions REMOVE WHAT WE DONT CALL
**/
...
function getRandomInt(min, max) {
  ...
  function loadFaceArray(trials) {//load 53 pictures that we need, shuffle, don't take 
    var FaceArray = []
  
    
    if (Face.bw == "b"){ //create practice trials based on bw division
        FaceArray.push([['img/merged_images/anger_125_H_v3.jpg']]);
        FaceArray.push([['img/merged_images/happy_75_H_v3.jpg']])
    } else {
        FaceArray.push([['img/merged_images/happy_75_D_v3.jpg']])
        FaceArray.push([['img/merged_images/anger_125_D_v3.jpg']])
      }
    ...
    for(i = 0; i < trials; i++){
        ...
        if (Face.bw == "w"){ // show white or black images based on bw random selection
            Face.personX = getRandomElement(['A','B','C','D']);
        } else {
            Face.personX = getRandomElement(['E','F','G','H']);
        }

        Face.verX = getRandomElement(["v1", "v2", "v3"])
        k = ['img/merged_images/'+ Face.emotionX + Face.numX +"_"+ Face.personX +"_" + Face.verX +'.jpg']
        FaceArray.push([k]);
    }
    return FaceArray}
  ...
  function createSlideList(start,end){
    ...
    for (i = start; i < (end+1); i++){
      if (Face.bw == "w"){
        list.push( 'img/ins/race_task_slides/w/Slide ' + i + '.png');
      } else {
        list.push( 'img/ins/race_task_slides/b/Slide ' + i + '.png');
      }
    } 
  ...
  function checkResponse(data){ //check repeated response
    ...
    if (trialNumber > 30) { //after practice trials and two real task trials, we begin to test whether choice is the same as previous two
  ...
  // get the next value on the Face Array list

  function getNextFace () {  //use this function to shift to next value of list
    Face.currentFace = faceArrayList.shift();
  ...
  function getScale (){ //generate the rating scale depending on the person and valence randomly chosen in faceArray
    var scale = [];
    s = String(Face.currentFace)
    p = s.replace(/([^A-Z])/g, "")
    Face.personX = p
    

    
    if (s.includes("anger_")) {
      Face.emotionX = 100
      Face.posX = Number(s.match(/([0-9]{2,3})/g)[0])-100

      for(i = 1; i < (50+1); i++){
        scale.push('img/'+ p + (100 +i) + '.jpg')}
    } else {
      Face.emotionX = 50
      Face.posX = Number(s.match(/([0-9]{2,3})/g)[0])-50

      for(i = 1; i < (50+1); i++){
        scale.push('img/'+ p+ (50 +i) + '.jpg')}
    }
  ...
  function getNextSlide () {  //use this function to shift to next value of list
